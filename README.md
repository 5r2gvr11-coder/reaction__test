...
function startScoreTicker() {
    ...
    tickerId = setInterval(() => {
        if (currentDisplayScore < targetScore) {
            let diff = targetScore - currentDisplayScore;
            let step = Math.ceil(diff / 5);
            currentDisplayScore += step;
            scoreEl.innerText = currentDisplayScore.toString().padStart(6, '0');
            scoreEl.classList.add("score-bump");
            setTimeout(() => scoreEl.classList.remove("score-bump"), 50);
        } else if (currentDisplayScore > targetScore) {
            // 用於練習切換正式時重設分數
...
function nextRound() {
    // 練習結束，切換到正式挑戰
    if (isPractice && total >= 10) {
        isPractice = false;
        total = 0; score = 0; targetScore = 0; mistakes = 0; missed = 0; reactionTimes = []; combo = 0;
        sounds.levelUp();
        modeTag.innerText = "FINAL CHALLENGE!";
        modeTag.style.background = "#5C9400";
        setTimeout(() => { modeTag.style.display = "none"; nextRound(); }, 1500);
        return;
    }

    if (!isPractice && total >= 100) { endTest(); return; }
    document.getElementById("progress-bar").style.width = isPractice ? (total*10) + "%" : total + "%";
    ...
    setTimeout(() => {
        if (!running) return;
        const letters = "ABCDEFGHJKLMNPQRSTUVWYZ";
        let char = letters[Math.floor(Math.random() * letters.length)];
        let isX = (Math.random() < 0.3 && !lastWasTarget);
        if (isX) char = "X";
        const r = Math.random();
        let color = r < 0.2 ? "yellow" : (r < 0.4 ? "red" : "white");
        ...
        let speed = isPractice ? 1400 : Math.max(500, 1100 - (total * 8));
        
        clearTimeout(timer);
        timer = setTimeout(() => {
            if (canClick) {
                // 練習模式不計入 missed
                if (currentIsTarget && !isPractice) { missed++; combo = 0; sounds.fall(); }
                else if (currentIsTarget && isPractice) { sounds.fall(); }
                canClick = false;
                nextRound();
            }
        }, speed);
...
function handleInput() {
    if (!running || !canClick) { 
        if(running && !isPractice) { mistakes++; sounds.bump(); } 
        else if(running && isPractice) { sounds.bump(); } // 練習模式只響鈴不計數
        return; 
    }
    const letterDiv = document.getElementById("letter");
    letterDiv.style.transform = "scale(0.8)";
    setTimeout(() => { letterDiv.style.transform = "scale(1)"; }, 50);
    clickCount++;
    if (currentIsTarget) {
        if (currentIsYellow) {
            if (clickCount === 1) {
                ...
                timer = setTimeout(() => {
                    if (canClick) { 
                        if(!isPractice) { missed++; combo = 0; }
                        sounds.fall(); canClick = false; nextRound(); 
                    }
                }, 800);
            } else if (clickCount === 2) { processHit(); }
        } else { processHit(); }
    } else {
        if(!isPractice) { mistakes++; combo = 0; }
...
function processHit() {
    ...
    if(!isPractice) {
        ...
        let bonus = (100 + Math.min(combo, 20) * 50);
        targetScore += bonus;

        const pop = document.createElement("div");
        pop.className = "score-pop";
        pop.innerText = "+" + bonus;
        pop.style.left = "calc(50% - 30px)";
        pop.style.top = "40%";
        document.getElementById("letter-container").appendChild(pop);
        setTimeout(() => pop.remove(), 500);
    }
    
    const marioCont = document.getElementById('mario-container');
    marioCont.style.transform = "translateY(-50px)";
    setTimeout(() => { marioCont.style.transform = "translateY(0)"; }, 150);
...
function endTest() {
    running = false; clearTimeout(timer); clearInterval(tickerId); bgm.pause();
    marioSprite.style.animationPlayState = 'paused';
    currentDisplayScore = targetScore;
    ...
    document.getElementById("res-rt").innerText = avg + "ms";
...
window.addEventListener("mousedown", (e) => { if(e.target.tagName !== 'BUTTON') { e.preventDefault(); handleInput(); } });
</script>
</body>
</html>
